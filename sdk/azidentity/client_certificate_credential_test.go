...
func TestClientCertificateCredential_NoData(t *testing.T) {
	...
	_, err := NewClientCertificateCredential(tenantID, clientID, []*x509.Certificate{}, key, nil)
	if err == nil {
...
func TestClientCertificateCredential_NoCertificate(t *testing.T) {
	test := allCertTests[0]
	_, err := NewClientCertificateCredential(tenantID, clientID, []*x509.Certificate{}, test.key, nil)
	if err == nil {
		t.Fatalf("Expected an error but received nil")
	}
}
